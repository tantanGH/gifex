...
int32_t buffer_open(BUFFER_HANDLE* buf, FILE* fp) {
  ...
  buf->rofs = 0;
  buf->wofs = 0;
//  buf->buffer_data = himem_malloc(buf->buffer_size, buf->use_high_memory);    // this works with 060turbo only for read ops
...
//    himem_free(buf->buffer_data, buf->use_high_memory);   // this works with 060turbo only
...
void buffer_read(BUFFER_HANDLE* buf, uint8_t* dest_ptr, size_t len) {
  ...
  if ((buf->rofs + len) <= buf->buffer_size) {
    ...
    int32_t available = buf->buffer_size - buf->rofs;
...
//  write offset rewind
//
void buffer_write_skip(BUFFER_HANDLE* buf, size_t len) {
  buf->wofs += len;
...
//  return written size
//
int32_t buffer_written_size(BUFFER_HANDLE* buf) {
  return buf->wofs;
}
...
//
int32_t buffer_writable_size(BUFFER_HANDLE* buf) {
...
//
uint8_t* buffer_data(BUFFER_HANDLE* buf) {
