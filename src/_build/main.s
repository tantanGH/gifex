...
_show_help_message:					*show_help_message:
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	|,
							*| main.c:719:   printf("GIFEX - GIF image loader with XEiJ graphic extension support version " VERSION " by tantan 2022\n");
	pea _?LC0					*	pea .LC0		|
	lea _puts,a3					*	lea puts,%a3	|, tmp31
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:720:   printf("usage: gifex.x [options] <image.gif>\n");
	move.l #_?LC1,(sp)				*	move.l #.LC1,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:721:   printf("options:\n");
	move.l #_?LC2,(sp)				*	move.l #.LC2,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:722:   printf("   -c ... clear graphic screen\n");
	move.l #_?LC3,(sp)				*	move.l #.LC3,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:723:   printf("   -s<n> ... screen mode 0:384x256, 1:512x512, 2:768x512(512x512), 3:768x512(full,XEiJ only)\n");
	move.l #_?LC4,(sp)				*	move.l #.LC4,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:724:   printf("   -h ... show this help message\n");
	move.l #_?LC5,(sp)				*	move.l #.LC5,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:725:   printf("   -i ... show file information\n");
	move.l #_?LC6,(sp)				*	move.l #.LC6,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:726:   printf("   -n ... image centering\n");
	move.l #_?LC7,(sp)				*	move.l #.LC7,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:727:   printf("   -m ... memory cache mode\n");
	move.l #_?LC8,(sp)				*	move.l #.LC8,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:728:   printf("   -u ... use high memory for buffering\n");
	move.l #_?LC9,(sp)				*	move.l #.LC9,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:729:   printf("   -k ... wait key input\n");
	move.l #_?LC10,(sp)				*	move.l #.LC10,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:730:   printf("   -f<n> ... max number of images\n");
	move.l #_?LC11,(sp)				*	move.l #.LC11,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:731:   printf("   -o<x,y> ... display position\n");
	move.l #_?LC12,(sp)				*	move.l #.LC12,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:732:   printf("   -v<n> ... brightness (0-100)\n");
	move.l #_?LC13,(sp)				*	move.l #.LC13,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:733:   printf("   -b<n> ... buffer memory size factor[1-16] (default:8)\n");
...
_output_image:						*output_image:
	...
	move.l 64(sp),a3				*	move.l 64(%sp),%a3	| image_frame, image_frame
							*| main.c:95:   unsigned char* buffer = image_frame->frame_data_ptr;
	move.l 548(a3),d7				*	move.l 548(%a3),%d7	| image_frame_51(D)->frame_data_ptr, buffer
							*| main.c:96:   unsigned char* buffer_end = buffer + image_frame->frame_data_size;
	...
	move.l d2,48(sp)				*	move.l %d2,48(%sp)	|, %sfp
							*| main.c:98:   int width = image_block->width, height = image_block->height;
	...
	move.l d2,56(sp)				*	move.l %d2,56(%sp)	|, %sfp
							*| main.c:99:   int offset_x = g_start_x + image_block->left_position;
	...
	add.l _g_start_x,d3				*	add.l g_start_x,%d3	| g_start_x, offset_x
							*| main.c:100:   int offset_y = g_start_y + image_block->top_position;
...
_?L3:							*.L3:
							*| main.c:105:   unsigned short* global_palette = image_frame->global_color_table_ptr;
	move.l 8(a3),a4					*	move.l 8(%a3),%a4	| image_frame_51(D)->global_color_table_ptr, global_palette
							*| main.c:106:   unsigned short* palette = (image_block->flag_code & 0x01) ? image_frame->local_color_table : global_palette;
...
_output_image_vdisp:					*output_image_vdisp:
	...
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:146:     if (g_current_frame_index >= g_max_frame_index) {
	...
	jblt _?L22					*	jlt .L22		|
							*| main.c:147:       g_current_frame_index = 0;
...
_?L27:							*.L27:
	...
	jbne _?L45					*	jne .L45		|
							*| main.c:798:   if (gif_file_name == NULL) {
	tst.l d5					*	tst.l %d5	| gif_file_name
	jbne _?L46					*	jne .L46		|
							*| main.c:799:     printf("error: no input file.\n");
	pea _?LC19					*	pea .LC19		|
...
_?L30:							*.L30:
	...
	jbra _?L27					*	jra .L27		|
_?L33:							*.L33:
							*| main.c:766:         g_screen_mode = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:766:         g_screen_mode = atoi(argv[i]+2);
	...
	jbra _?L27					*	jra .L27		|
_?L39:							*.L39:
							*| main.c:768:         g_image_index_limit = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:768:         g_image_index_limit = atoi(argv[i]+2);
...
_?L163:							*.L163:
	...
	jbra _?L26					*	jra .L26		|
_?L29:							*.L29:
							*| main.c:786:       } else if (argv[i][1] == 'h') {
	cmp.b #104,d1					*	cmp.b #104,%d1	|, _6
	jbne _?L43					*	jne .L43		|
							*| main.c:787:         show_help_message();
	jbsr _show_help_message				*	jsr show_help_message		|
_?L44:							*.L44:
							*| main.c:788:         return 0;
...
_?L24:							*.L24:
	...
	rts						*	rts
_?L43:							*.L43:
							*| main.c:790:         printf("error: unknown option (%s).\n",argv[i]);
...
_?L115:							*.L115:
	...
	jbra _?L27					*	jra .L27		|
_?L46:							*.L46:
							*| main.c:818:   g_input_buffer_size = 65536 * g_buffer_memory_size_factor;
	...
	move.l d6,_g_input_buffer_size			*	move.l %d6,g_input_buffer_size	| _24, g_input_buffer_size
							*| main.c:821:   g_output_buffer_size = 131072 * g_buffer_memory_size_factor;
	...
	move.l d0,_g_output_buffer_size			*	move.l %d0,g_output_buffer_size	| tmp346, g_output_buffer_size
							*| main.c:824:   g_actual_width = g_screen_mode == 3 ? ACTUAL_WIDTH_EX : ACTUAL_WIDTH;
...
_?L47:							*.L47:
	...
	jbeq _?L49					*	jeq .L49		|
							*| main.c:631:       printf("error: file read error (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC20					*	pea .LC20		|
_?L164:							*.L164:
							*| main.c:638:       printf("error: cannot open input file (%s).\n", gif_file_name);
	jbsr _printf					*	jsr printf		|
_?L155:							*.L155:
							*| main.c:708:       free__(file_buffer_ptr, g_high_memory_mode);
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:712:     return 0;
	jbra _?L44					*	jra .L44		|
_?L49:							*.L49:
							*| main.c:636:     fp = fopen(gif_file_name, "rb");
	pea _?LC21					*	pea .LC21		|
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	jbsr _fopen					*	jsr fopen		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d3					*	move.l %d0,%d3	| tmp995, fp
							*| main.c:637:     if (fp == NULL) {
	jbne _?L51					*	jne .L51		|
							*| main.c:638:       printf("error: cannot open input file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC22					*	pea .LC22		|
...
_?L51:							*.L51:
							*| main.c:643:     file_buffer_ptr = malloc__(file_buffer_size, g_high_memory_mode);
	...
	lea _fclose,a5					*	lea fclose,%a5	|, tmp966
							*| main.c:644:     if (file_buffer_ptr == NULL) {
	jbne _?L52					*	jne .L52		|
							*| main.c:645:       printf("error: cannot allocate memory for file buffer.\n");
	pea _?LC23					*	pea .LC23		|
	jbsr _puts					*	jsr puts		|
							*| main.c:646:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	jbsr (a5)					*	jsr (%a5)		| tmp966
_?L154:							*.L154:
							*| main.c:863:   C_FNKMOD(func_key_display_mode);
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:865:   return 0;
	jbra _?L44					*	jra .L44		|
_?L52:							*.L52:
							*| main.c:651:     read_size = fread(file_buffer_ptr, 1, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	pea 1.w						*	pea 1.w		|
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:652:     if (read_size <= 0) {
	tst.l d0					*	tst.l %d0	| tmp997
	jbgt _?L53					*	jgt .L53		|
							*| main.c:653:       printf("error: file read error.\n");
	pea _?LC24					*	pea .LC24		|
	jbsr _puts					*	jsr puts		|
							*| main.c:654:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
_?L153:							*.L153:
							*| main.c:702:       fclose(fp);
	jbsr (a5)					*	jsr (%a5)		| tmp966
							*| main.c:708:       free__(file_buffer_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
...
_?L53:							*.L53:
							*| main.c:660:     memcpy_buffer(header.signature, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	...
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:661:     header.signature[3] = '\0';
	clr.b -95(a6)					*	clr.b -95(%fp)	| MEM[(struct  *)_408].signature[3]
							*| main.c:662:     if (strcmp(header.signature, "GIF") != 0 ) {
	pea _?LC25					*	pea .LC25		|
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp360,
	...
	jbeq _?L54					*	jeq .L54		|
							*| main.c:663:       printf("error: signature error. not a GIF file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC26					*	pea .LC26		|
_?L152:							*.L152:
							*| main.c:698:     printf("  aspect ratio: %d\n", header.aspect_ratio);
...
_?L54:							*.L54:
							*| main.c:670:     memcpy_buffer(header.version, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	...
	jbsr (a0)					*	jsr (%a0)		| tmp361
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:671:     header.version[3] = '\0';
	clr.b -91(a6)					*	clr.b -91(%fp)	| MEM[(struct  *)_408].version[3]
							*| main.c:672:     if (strcmp(header.version, "89a") != 0 ) {
	pea _?LC27					*	pea .LC27		|
	move.l d7,-(sp)					*	move.l %d7,-(%sp)	| tmp370,
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	...
	jbeq _?L55					*	jeq .L55		|
							*| main.c:673:       printf("error: unsupported GIF version (%s).\n", header.version);
	move.l d7,-(sp)					*	move.l %d7,-(%sp)	| tmp370,
	pea _?LC28					*	pea .LC28		|
	jbra _?L152					*	jra .L152		|
_?L55:							*.L55:
							*| main.c:680:     header.screen_width   = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,-90(a6)				*	move.w %d0,-90(%fp)	| tmp1000, MEM[(struct  *)_408].screen_width
							*| main.c:681:     header.screen_height  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,-88(a6)				*	move.w %d0,-88(%fp)	| tmp1001, MEM[(struct  *)_408].screen_height
							*| main.c:682:     header.flag_code      = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-86(a6)				*	move.b %d0,-86(%fp)	| tmp1002, MEM[(struct  *)_408].flag_code
							*| main.c:683:     header.bg_color_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-85(a6)				*	move.b %d0,-85(%fp)	| tmp1003, MEM[(struct  *)_408].bg_color_index
							*| main.c:684:     header.aspect_ratio   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp359,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	pea _?LC31					*	pea .LC31		|
	jbsr (a3)					*	jsr (%a3)		| tmp968
							*| main.c:690:     printf("   GIF version: %s\n", header.version);
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
	move.l d7,(sp)					*	move.l %d7,(%sp)	| tmp370,
	pea _?LC32					*	pea .LC32		|
	jbsr (a3)					*	jsr (%a3)		| tmp968
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:691:     printf("  screen width: %d\n", header.screen_width);
	...
	jbsr (a3)					*	jsr (%a3)		| tmp968
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:692:     printf(" screen height: %d\n", header.screen_height);
	...
	move.b -86(a6),d0				*	move.b -86(%fp),%d0	| MEM[(struct  *)_408].flag_code, MEM[(struct  *)_408].flag_code
							*| main.c:74:   return ( byte_data >> bit_ofs ) & ( 0xff >> ( 8 - bit_len ));
...
_?L57:							*.L57:
	...
	jbsr (a3)					*	jsr (%a3)		| tmp968
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:697:     printf("BG color index: %d\n", header.bg_color_index);
...
_?L48:							*.L48:
	...
	lsl.l #5,d3					*	lsl.l #5,%d3	|, _601
							*| main.c:82:     unsigned int c = (int)(i * 32 * g_brightness / 100) >> 8;
	moveq #0,d4					*	moveq #0,%d4	| ivtmp.145
	moveq #0,d6					*	moveq #0,%d6	| ivtmp.138
							*| main.c:83:     g_rgb555_r[i] = (unsigned short)((c <<  6) + 1);
	lea _g_rgb555_r,a5				*	lea g_rgb555_r,%a5	|, tmp450
							*| main.c:84:     g_rgb555_g[i] = (unsigned short)((c << 11) + 1);
	lea _g_rgb555_g,a4				*	lea g_rgb555_g,%a4	|, tmp452
							*| main.c:85:     g_rgb555_b[i] = (unsigned short)((c <<  1) + 1);
...
_?L60:							*.L60:
	...
	jbeq _?L61					*	jeq .L61		|
							*| main.c:178:       printf("error: file read error (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC20					*	pea .LC20		|
_?L160:							*.L160:
							*| main.c:185:       printf("error: cannot open input file (%s).\n", gif_file_name);
	jbsr _printf					*	jsr printf		|
_?L161:							*.L161:
	addq.l #8,sp					*	addq.l #8,%sp	|,
_?L62:							*.L62:
...
_?L61:							*.L61:
							*| main.c:183:     fp = fopen(gif_file_name, "rb");
	pea _?LC21					*	pea .LC21		|
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	jbsr _fopen					*	jsr fopen		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d3					*	move.l %d0,%d3	| tmp1008, fp
							*| main.c:184:     if (fp == NULL) {
	jbne _?L63					*	jne .L63		|
							*| main.c:185:       printf("error: cannot open input file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC22					*	pea .LC22		|
...
_?L63:							*.L63:
							*| main.c:190:     file_buffer_ptr = malloc__(file_buffer_size, g_high_memory_mode);
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	jbsr _malloc__					*	jsr malloc__		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d4					*	move.l %d0,%d4	| tmp1009, file_buffer_ptr
							*| main.c:191:     if (file_buffer_ptr == NULL) {
	jbne _?L64					*	jne .L64		|
							*| main.c:192:       printf("error: cannot allocate memory for file buffer.\n");
	pea _?LC23					*	pea .LC23		|
_?L162:							*.L162:
							*| main.c:250:       printf("error: cannot allocate memory for decode buffer.\n");
	jbsr _puts					*	jsr puts		|
							*| main.c:251:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	...
	jbra _?L62					*	jra .L62		|
_?L64:							*.L64:
							*| main.c:198:     read_size = fread(file_buffer_ptr, 1, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea 1.w						*	pea 1.w		|
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| file_buffer_ptr,
	jbsr _fread					*	jsr fread		|
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:199:     if (read_size <= 0) {
	tst.l d0					*	tst.l %d0	| tmp1010
	jbgt _?L65					*	jgt .L65		|
							*| main.c:200:       printf("error: file read error.\n");
	pea _?LC24					*	pea .LC24		|
	jbsr _puts					*	jsr puts		|
							*| main.c:201:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
_?L165:							*.L165:
							*| main.c:221:       fclose(fp);
	jbsr _fclose					*	jsr fclose		|
							*| main.c:222:       free__(file_buffer_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr _free__					*	jsr free__		|
	jbra _?L161					*	jra .L161		|
_?L65:							*.L65:
							*| main.c:207:     memcpy_buffer(header.signature, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	...
	jbsr (a0)					*	jsr (%a0)		|
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:208:     header.signature[3] = '\0';
	clr.b -111(a6)					*	clr.b -111(%fp)	| MEM[(struct  *)_279].signature[3]
							*| main.c:209:     if (strcmp(header.signature, "GIF") != 0 ) {
	pea _?LC25					*	pea .LC25		|
	pea -114(a6)					*	pea -114(%fp)		|
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	...
	jbeq _?L66					*	jeq .L66		|
							*| main.c:210:       printf("error: signature error. not a GIF file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC26					*	pea .LC26		|
_?L166:							*.L166:
							*| main.c:220:       printf("error: unsupported GIF version (%s).\n", header.version);
	jbsr _printf					*	jsr printf		|
							*| main.c:221:       fclose(fp);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
...
_?L66:							*.L66:
							*| main.c:217:     memcpy_buffer(header.version, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	...
	jbsr (a2)					*	jsr (%a2)		|
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:218:     header.version[3] = '\0';
	clr.b -107(a6)					*	clr.b -107(%fp)	| MEM[(struct  *)_279].version[3]
							*| main.c:219:     if (strcmp(header.version,"89a") != 0 ) {
	pea _?LC27					*	pea .LC27		|
	pea (a6,d5.l)					*	pea (%fp,%d5.l)		|
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
...
_?L67:							*.L67:
							*| main.c:227:     header.screen_width   = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,-106(a6)				*	move.w %d0,-106(%fp)	| tmp1013, MEM[(struct  *)_279].screen_width
							*| main.c:228:     header.screen_height  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,-104(a6)				*	move.w %d0,-104(%fp)	| tmp1014, MEM[(struct  *)_279].screen_height
							*| main.c:229:     header.flag_code      = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-102(a6)				*	move.b %d0,-102(%fp)	| tmp1015, MEM[(struct  *)_279].flag_code
							*| main.c:230:     header.bg_color_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-101(a6)				*	move.b %d0,-101(%fp)	| tmp1016, MEM[(struct  *)_279].bg_color_index
							*| main.c:231:     header.aspect_ratio   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-100(a6)				*	move.b %d0,-100(%fp)	| tmp1017, MEM[(struct  *)_279].aspect_ratio
							*| main.c:234:     if (read_bits(header.flag_code,7,1)) {
	move.b -102(a6),d0				*	move.b -102(%fp),%d0	| MEM[(struct  *)_279].flag_code, _192
							*| main.c:234:     if (read_bits(header.flag_code,7,1)) {
	tst.b d0					*	tst.b %d0	| _192
	jbmi _?L68					*	jmi .L68		|
_?L72:							*.L72:
							*| main.c:248:     decode_buffer_ptr = malloc__(g_output_buffer_size, g_high_memory_mode);
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	...
	move.l d0,-134(a6)				*	move.l %d0,-134(%fp)	| tmp1018, %sfp
							*| main.c:249:     if (decode_buffer_ptr == NULL) {
	jbeq _?L69					*	jeq .L69		|
							*| main.c:170:     int image_frame_index = 0;
	moveq #0,d5					*	moveq #0,%d5	| image_frame_index
_?L70:							*.L70:
							*| main.c:259:       unsigned char block_type = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:261:       if (block_type == GIF_TRAILOR) {
	cmp.b #59,d0					*	cmp.b #59,%d0	|, block_type
	jbeq _?L73					*	jeq .L73		|
							*| main.c:270:       } else if (block_type == IMAGE_SEPARATOR) {
	cmp.b #44,d0					*	cmp.b #44,%d0	|, block_type
	jbne _?L74					*	jne .L74		|
							*| main.c:278:         image_frame->bg_color_index = header.bg_color_index;
	...
	move.l d0,4(a5)					*	move.l %d0,4(%a5)	| MEM[(struct  *)_279].bg_color_index, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_FRAME *)&g_image_frames][image_frame_index_217].bg_color_index
							*| main.c:279:         image_frame->global_color_table_ptr = g_global_color_table;
	move.l #_g_global_color_table,8(a5)		*	move.l #g_global_color_table,8(%a5)	|, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_FRAME *)&g_image_frames][image_frame_index_217].global_color_table_ptr
							*| main.c:290:         image_block->separator     = block_type;
	move.b #44,20(a5)				*	move.b #44,20(%a5)	|, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.separator
							*| main.c:291:         image_block->left_position = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,22(a5)				*	move.w %d0,22(%a5)	| tmp1023, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.left_position
							*| main.c:292:         image_block->top_position  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,24(a5)				*	move.w %d0,24(%a5)	| tmp1024, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.top_position
							*| main.c:293:         image_block->width         = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,26(a5)				*	move.w %d0,26(%a5)	| tmp1025, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.width
							*| main.c:294:         image_block->height        = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,28(a5)				*	move.w %d0,28(%a5)	| tmp1026, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.height
							*| main.c:295:         image_block->flag_code     = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
...
_?L167:							*.L167:
	...
	move.l d1,_g_start_x				*	move.l %d1,g_start_x	| tmp631, g_start_x
							*| main.c:302:               g_start_y = (256 - image_block->height) / 2;
...
_?L156:							*.L156:
							*| main.c:308:               g_start_y = (512 - image_block->height) / 2;
...
_?L79:							*.L79:
							*| main.c:310:             if (g_start_x < 0) g_start_x = 0;
	tst.l _g_start_x				*	tst.l g_start_x	| g_start_x
	jbge _?L81					*	jge .L81		|
							*| main.c:310:             if (g_start_x < 0) g_start_x = 0;
	clr.l _g_start_x				*	clr.l g_start_x	| g_start_x
_?L81:							*.L81:
							*| main.c:311:             if (g_start_y < 0) g_start_y = 0;
	tst.l _g_start_y				*	tst.l g_start_y	| g_start_y
	jbge _?L76					*	jge .L76		|
							*| main.c:311:             if (g_start_y < 0) g_start_y = 0;
	clr.l _g_start_y				*	clr.l g_start_y	| g_start_y
_?L76:							*.L76:
							*| main.c:316:         if (read_bits(image_block->flag_code,7,1)) {
	tst.b d0					*	tst.b %d0	| _225
	jbmi _?L82					*	jmi .L82		|
_?L85:							*.L85:
							*| main.c:327:         image_block->lzw_min_code_size = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,31(a0)				*	move.b %d0,31(%a0)	| _275, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.lzw_min_code_size
							*| main.c:330:         pixel_count = (image_block->width) * (image_block->height);
	...
	move.l d1,-126(a6)				*	move.l %d1,-126(%fp)	|, %sfp
							*| main.c:333:         initialize_lzw(&lzw, image_block->lzw_min_code_size);
	and.l #255,d0					*	and.l #255,%d0	|, _275
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| _275,
	pea -98(a6)					*	pea -98(%fp)		|
	jbsr _initialize_lzw				*	jsr initialize_lzw		|
							*| main.c:335:         sub_block_data2 = malloc__(1024*1024,g_high_memory_mode);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	...
	move.l d0,a3					*	move.l %d0,%a3	| tmp1029, sub_block_data2
							*| main.c:287:         int sub_block_ofs2 = 0;
...
_?L83:							*.L83:
	...
	jbne _?L86					*	jne .L86		|
							*| main.c:361:         decode_buffer_ofs = decode_lzw(&lzw,sub_block_data2,sub_block_ofs2,decode_buffer_ptr,pixel_count);
	...
	move.l d0,a4					*	move.l %d0,%a4	| tmp1034, decode_buffer_ofs
							*| main.c:363:         if (g_memory_cache_mode) {
	tst.l _g_memory_cache_mode			*	tst.l g_memory_cache_mode	| g_memory_cache_mode
	jbne _?L87					*	jne .L87		|
							*| main.c:387:           image_frame->frame_data_ptr = decode_buffer_ptr;
	...
	move.l -134(a6),(a5)				*	move.l -134(%fp),(%a5)	| %sfp, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_FRAME *)&g_image_frames][image_frame_index_217].frame_data_ptr
							*| main.c:388:           image_frame->frame_data_size = decode_buffer_ofs;
	move.l a4,544(a0)				*	move.l %a4,544(%a0)	| decode_buffer_ofs, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_FRAME *)&g_image_frames][image_frame_index_217].frame_data_size
							*| main.c:390:           output_image(image_frame);
	move.l a0,-(sp)					*	move.l %a0,-(%sp)	| tmp771,
	jbsr _output_image				*	jsr output_image		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:392:           image_frame->frame_data_ptr = NULL;
...
_?L68:							*.L68:
	lea _g_global_color_table,a0			*	lea g_global_color_table,%a0	|, ivtmp.132
							*| main.c:235:       int gct_size = 1 << (1 + read_bits(header.flag_code,0,3));
	moveq #7,d1					*	moveq #7,%d1	|,
...
_?L71:							*.L71:
							*| main.c:237:         unsigned char r = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,d7					*	move.b %d0,%d7	| tmp1019, r
							*| main.c:238:         unsigned char g = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,d5					*	move.b %d0,%d5	| tmp1020, g
							*| main.c:239:         unsigned char b = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr _get_uchar_buffer				*	jsr get_uchar_buffer		|
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:240:         g_global_color_table[i] = g_rgb555_r[ r ] | g_rgb555_g[ g ] | g_rgb555_b[ b ];
	...
	move.w d1,(a0)+					*	move.w %d1,(%a0)+	| tmp527, MEM[(short unsigned int *)_591]
							*| main.c:236:       for (int i = 0; i < gct_size; i++) {
...
_?L78:							*.L78:
							*| main.c:303:             } else if (g_screen_mode == 1 || g_screen_mode == 2) {
	...
	jbcs _?L80					*	jcs .L80		|
							*| main.c:304:               g_start_x = (512 - image_block->width) / 2;
	moveq #0,d2					*	moveq #0,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
	move.w _g_image_frames+26,d2			*	move.w g_image_frames+26,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
							*| main.c:304:               g_start_x = (512 - image_block->width) / 2;
	move.l #512,d1					*	move.l #512,%d1	|, tmp641
_?L157:							*.L157:
							*| main.c:307:               g_start_x = (768 - image_block->width) / 2;
...
_?L80:							*.L80:
							*| main.c:306:             } else if (g_screen_mode == 3) {
...
_?L82:							*.L82:
							*| main.c:317:           int lct_size = 1 << (1 + (read_bits(image_block->flag_code,0,3)));
	moveq #7,d2					*	moveq #7,%d2	|,
	...
	add.l #_g_image_frames+32,a0			*	add.l #g_image_frames+32,%a0	|, ivtmp.128
							*| main.c:318:           for (int i = 0; i < lct_size; i++) {
	clr.l -142(a6)					*	clr.l -142(%fp)	| %sfp
							*| main.c:319:             unsigned char r = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
...
_?L84:							*.L84:
	...
	move.w d0,a3					*	move.w %d0,%a3	| tmp1030, r
							*| main.c:320:             unsigned char g = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a5,-(sp)					*	move.l %a5,-(%sp)	| tmp713,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,d7					*	move.b %d0,%d7	| tmp1031, g
							*| main.c:321:             unsigned char b = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a5,-(sp)					*	move.l %a5,-(%sp)	| tmp713,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp714
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:322:             image_frame->local_color_table[i] = g_rgb555_r[ r ] + g_rgb555_g[ g ] + g_rgb555_b[ b ];
	...
	jbra _?L85					*	jra .L85		|
_?L86:							*.L86:
							*| main.c:355:             memcpy_buffer(sub_block_data2 + sub_block_ofs2, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, sub_block_size, fp);
	...
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:356:             sub_block_ofs2 += sub_block_size;
	move.l -138(a6),d0				*	move.l -138(%fp),%d0	|,
	add.l d0,a4					*	add.l %d0,%a4	| _285, sub_block_ofs2
	jbra _?L83					*	jra .L83		|
_?L87:							*.L87:
							*| main.c:367:           if ((image_frame->frame_data_ptr = malloc__(size,g_high_memory_mode)) == NULL) {
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	...
	move.l d0,548(a5)				*	move.l %d0,548(%a5)	| _296, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_FRAME *)&g_image_frames][image_frame_index_217].frame_data_ptr
							*| main.c:367:           if ((image_frame->frame_data_ptr = malloc__(size,g_high_memory_mode)) == NULL) {
	jbne _?L89					*	jne .L89		|
							*| main.c:368:             printf("error: memory allocation failure (errno=%d).\n",errno);
	move.l _errno,d0				*	move.l errno,%d0	| errno, errno.79_297
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| errno.79_297,
	pea _?LC42					*	pea .LC42		|
_?L158:							*.L158:
							*| main.c:528:         printf("error: unknown block (0x%02X).\n", block_type);
	jbsr _printf					*	jsr printf		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:530:         break;
	jbra _?L73					*	jra .L73		|
_?L89:							*.L89:
							*| main.c:373:           memcpy(image_frame->frame_data_ptr,decode_buffer_ptr,size);
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| decode_buffer_ofs,
	move.l -134(a6),-(sp)				*	move.l -134(%fp),-(%sp)	| %sfp,
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| _296,
	jbsr _memcpy					*	jsr memcpy		|
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
							*| main.c:374:           image_frame->frame_data_size = size;
...
_?L91:							*.L91:
							*| main.c:396:         free__(sub_block_data2, g_high_memory_mode);
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| sub_block_data2,
	jbsr _free__					*	jsr free__		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:401:         image_frame_index++;
	addq.l #1,d5					*	addq.l #1,%d5	|, image_frame_index
							*| main.c:402:         if (image_frame_index >= g_image_index_limit || image_frame_index >= MAX_IMAGE_FRAMES) {
	cmp.l _g_image_index_limit.l,d5			*	cmp.l g_image_index_limit.l,%d5	| g_image_index_limit, image_frame_index
	jbge _?L73					*	jge .L73		|
							*| main.c:406:         if (g_memory_cache_mode && (image_frame_index % 30) == 0) {
	...
	jbsr _putchar					*	jsr putchar		|
							*| main.c:408:           fflush(stdout);
	move.l #__iob+26,(sp)				*	move.l #_iob+26,(%sp)	|,
	jbsr _fflush					*	jsr fflush		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
	jbra _?L70					*	jra .L70		|
_?L74:							*.L74:
							*| main.c:411:       } else if (block_type == EXTENSION_INTRODUCER) {
	cmp.b #33,d0					*	cmp.b #33,%d0	|, block_type
	jbne _?L94					*	jne .L94		|
							*| main.c:413:         unsigned char extension_label = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:415:         if (extension_label == GRAPHIC_CONTROL_LABEL) {
	cmp.b #-7,d0					*	cmp.b #-7,%d0	|, extension_label
	jbne _?L95					*	jne .L95		|
							*| main.c:424:           graphic_ctrl_ext->introducer  = block_type;
	...
	move.w #8697,12(a5,d7.l)			*	move.w #8697,12(%a5,%d7.l)	|, MEM <unsigned short> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)_258 + 12B]
							*| main.c:427:           graphic_ctrl_ext->block_size        = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,14(a5)				*	move.b %d0,14(%a5)	| tmp1038, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.block_size
							*| main.c:428:           graphic_ctrl_ext->flag_code         = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,15(a5)				*	move.b %d0,15(%a5)	| tmp1039, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.flag_code
							*| main.c:429:           graphic_ctrl_ext->delay_time        = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.w d0,16(a5)				*	move.w %d0,16(%a5)	| tmp1040, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.delay_time
							*| main.c:430:           graphic_ctrl_ext->transparent_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,18(a5)				*	move.b %d0,18(%a5)	| tmp1041, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.transparent_index
							*| main.c:433:           graphic_ctrl_ext->terminator = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,19(a5)				*	move.b %d0,19(%a5)	| _308, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.terminator
							*| main.c:434:           if (graphic_ctrl_ext->terminator != 0x00) {
	jbeq _?L70					*	jeq .L70		|
							*| main.c:435:             printf("error: graphic control extension read error.\n");
	pea _?LC43					*	pea .LC43		|
_?L159:							*.L159:
							*| main.c:458:             printf("error: comment extension read error.\n");
	jbsr _puts					*	jsr puts		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
_?L73:							*.L73:
							*| main.c:536:     if (g_memory_cache_mode) {
	tst.l _g_memory_cache_mode			*	tst.l g_memory_cache_mode	| g_memory_cache_mode
	jbne _?L102					*	jne .L102		|
_?L112:							*.L112:
							*| main.c:584:       fclose(fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	jbsr _fclose					*	jsr fclose		|
							*| main.c:590:       free__(file_buffer_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea _free__,a3					*	lea free__,%a3	|, tmp940
	jbsr (a3)					*	jsr (%a3)		| tmp940
							*| main.c:596:         free__(decode_buffer_ptr, g_high_memory_mode);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	move.l -134(a6),-(sp)				*	move.l -134(%fp),-(%sp)	| %sfp,
	jbsr (a3)					*	jsr (%a3)		| tmp940
...
_?L103:							*.L103:
							*| main.c:601:     for (int i = 0; i < image_frame_index; i++) {
	cmp.l -130(a6),d5				*	cmp.l -130(%fp),%d5	| %sfp, image_frame_index
	jble _?L62					*	jle .L62		|
							*| main.c:602:         if (g_image_frames[i].frame_data_ptr != NULL) {
	move.l (a4),d0					*	move.l (%a4),%d0	| MEM[(unsigned char * *)_593], _341
							*| main.c:602:         if (g_image_frames[i].frame_data_ptr != NULL) {
	jbeq _?L113					*	jeq .L113		|
							*| main.c:603:             free__(g_image_frames[i].frame_data_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| _341,
	jbsr (a3)					*	jsr (%a3)		| tmp971
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:604:             g_image_frames[i].frame_data_ptr = NULL;
...
_?L95:							*.L95:
							*| main.c:440:         } else if (extension_label == COMMENT_LABEL) {
	cmp.b #-2,d0					*	cmp.b #-2,%d0	|, extension_label
	jbne _?L97					*	jne .L97		|
							*| main.c:452:           comment_ext.block_size = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
							*| main.c:453:           skip_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, comment_ext.block_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	...
	jbsr _skip_buffer				*	jsr skip_buffer		|
							*| main.c:456:           comment_ext.terminator = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	jbsr (a4)					*	jsr (%a4)		| tmp534
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:457:           if (comment_ext.terminator != 0x00) {
...
_?L97:							*.L97:
							*| main.c:463:         } else if (extension_label == PLAIN_TEXT_LABEL) {
	cmp.b #1,d0					*	cmp.b #1,%d0	|, extension_label
	jbne _?L99					*	jne .L99		|
							*| main.c:475:           plain_text_ext.block_size              = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
							*| main.c:476:           plain_text_ext.text_grid_left_position = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	jbsr (a5)					*	jsr (%a5)		| tmp898
							*| main.c:477:           plain_text_ext.text_grid_top_position  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a5)					*	jsr (%a5)		| tmp898
							*| main.c:478:           plain_text_ext.text_grid_width         = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a5)					*	jsr (%a5)		| tmp898
							*| main.c:479:           plain_text_ext.text_grid_height        = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a5)					*	jsr (%a5)		| tmp898
							*| main.c:480:           plain_text_ext.character_cell_width    = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
							*| main.c:481:           plain_text_ext.character_cell_height   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
							*| main.c:483:           plain_text_ext.block_size2 = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr (a4)					*	jsr (%a4)		| tmp534
							*| main.c:484:           skip_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, plain_text_ext.block_size2, fp);
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	...
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr _skip_buffer				*	jsr skip_buffer		|
							*| main.c:486:           plain_text_ext.terminator = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	jbsr (a4)					*	jsr (%a4)		| tmp534
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:487:           if (plain_text_ext.terminator != 0x00) {
	tst.b d0					*	tst.b %d0	| tmp1046
	jbeq _?L70					*	jeq .L70		|
							*| main.c:488:             printf("error: plain text extension read error.\n");
	pea _?LC45					*	pea .LC45		|
	jbra _?L159					*	jra .L159		|
_?L99:							*.L99:
							*| main.c:493:         } else if (extension_label == APPLICATION_LABEL) {
	cmp.b #-1,d0					*	cmp.b #-1,%d0	|, extension_label
	jbne _?L100					*	jne .L100		|
							*| main.c:500:           app_ext.introducer = block_type;
	move.w #8703,-98(a6)				*	move.w #8703,-98(%fp)	|, MEM <unsigned short> [(unsigned char *)_408]
							*| main.c:503:           app_ext.block_size = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	move.b d0,-96(a6)				*	move.b %d0,-96(%fp)	| tmp1047, MEM[(struct  *)_408].block_size
							*| main.c:505:           memcpy_buffer(app_ext.identifier, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 8, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 8.w						*	pea 8.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	pea -95(a6)					*	pea -95(%fp)		|
	jbsr _memcpy_buffer				*	jsr memcpy_buffer		|
							*| main.c:506:           memcpy_buffer(app_ext.auth_code,  file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	lea (20,sp),sp					*	lea (20,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	...
	pea -87(a6)					*	pea -87(%fp)		|
	jbsr _memcpy_buffer				*	jsr memcpy_buffer		|
							*| main.c:508:           app_ext.block_size2 = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (20,sp),sp					*	lea (20,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	move.b d0,-84(a6)				*	move.b %d0,-84(%fp)	| _316, MEM[(struct  *)_408].block_size2
							*| main.c:509:           skip_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, app_ext.block_size2, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	...
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	jbsr _skip_buffer				*	jsr skip_buffer		|
							*| main.c:511:           app_ext.terminator = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp533,
	...
	move.b d0,-78(a6)				*	move.b %d0,-78(%fp)	| _318, MEM[(struct  *)_408].terminator
							*| main.c:512:           if (app_ext.terminator != 0x00) {
	jbeq _?L70					*	jeq .L70		|
							*| main.c:513:             printf("error: application extension read error.\n");
	pea _?LC46					*	pea .LC46		|
	jbra _?L159					*	jra .L159		|
_?L100:							*.L100:
							*| main.c:520:           printf("error: unknown extension label (0x%02X).\n", extension_label);
...
_?L105:							*.L105:
							*| main.c:562:       while(!(*gpip & 0x10));     // wait while V-DISP is L
	...
	jbeq _?L105					*	jeq .L105		|
							*| main.c:564:       if (VDISPST((unsigned char*)output_image_vdisp, 0, 2) == 0) {
	pea 2.w						*	pea 2.w		|
	clr.l -(sp)					*	clr.l -(%sp)	|
...
_?L107:							*.L107:
							*| main.c:568:         while( (*gpip & 0x10));     // wait while V-DISP is H
...
_?L108:							*.L108:
							*| main.c:569:         while(!(*gpip & 0x10));     // wait while V-DISP is L
	...
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
_?L110:							*.L110:
							*| main.c:577:       while(!(*gpip & 0x10));     // wait while V-DISP is L
...
_?L111:							*.L111:
							*| main.c:578:       while( (*gpip & 0x10));     // wait while V-DISP is H
...
_?L106:							*.L106:
							*| main.c:574:         printf("error: cannot use vsync interrupt.\n");
	pea _?LC49					*	pea .LC49		|
	jbsr _puts					*	jsr puts		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
