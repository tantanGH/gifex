...
_show_help_message:					*show_help_message:
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	|,
							*| main.c:697:   printf("GIFEX - GIF image loader with XEiJ graphic extension support version " VERSION " by tantan 2022\n");
	pea _?LC0					*	pea .LC0		|
	lea _puts,a3					*	lea puts,%a3	|, tmp31
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:698:   printf("usage: gifex.x [options] <image.gif>\n");
	move.l #_?LC1,(sp)				*	move.l #.LC1,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:699:   printf("options:\n");
	move.l #_?LC2,(sp)				*	move.l #.LC2,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:700:   printf("   -c ... clear graphic screen\n");
	move.l #_?LC3,(sp)				*	move.l #.LC3,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:701:   printf("   -s<n> ... screen mode 0:384x256, 1:512x512, 2:768x512(512x512), 3:768x512(full,XEiJ only)\n");
	move.l #_?LC4,(sp)				*	move.l #.LC4,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:702:   printf("   -h ... show this help message\n");
	move.l #_?LC5,(sp)				*	move.l #.LC5,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:703:   printf("   -i ... show file information\n");
	move.l #_?LC6,(sp)				*	move.l #.LC6,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:704:   printf("   -n ... image centering\n");
	move.l #_?LC7,(sp)				*	move.l #.LC7,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:705:   printf("   -m ... memory cache mode\n");
	move.l #_?LC8,(sp)				*	move.l #.LC8,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:706:   printf("   -u ... use high memory for buffering\n");
	move.l #_?LC9,(sp)				*	move.l #.LC9,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:707:   printf("   -k ... wait key input\n");
	move.l #_?LC10,(sp)				*	move.l #.LC10,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:708:   printf("   -f<n> ... max number of images\n");
	move.l #_?LC11,(sp)				*	move.l #.LC11,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:709:   printf("   -o<x,y> ... display position\n");
	move.l #_?LC12,(sp)				*	move.l #.LC12,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:710:   printf("   -v<n> ... brightness (0-100)\n");
	move.l #_?LC13,(sp)				*	move.l #.LC13,(%sp)	|,
	jbsr (a3)					*	jsr (%a3)		| tmp31
							*| main.c:711:   printf("   -b<n> ... buffer memory size factor[1-16] (default:8)\n");
...
_?L27:							*.L27:
	...
	jbne _?L45					*	jne .L45		|
							*| main.c:776:   if (gif_file_name == NULL) {
	tst.l d5					*	tst.l %d5	| gif_file_name
	jbne _?L46					*	jne .L46		|
							*| main.c:777:     printf("error: no input file.\n");
	pea _?LC19					*	pea .LC19		|
	jbra _?L166					*	jra .L166		|
_?L28:							*.L28:
	move.b d1,d0					*	move.b %d1,%d0	| _6, tmp322
	add.b #-98,d0					*	add.b #-98,%d0	|, tmp322
	cmp.b #20,d0					*	cmp.b #20,%d0	|, tmp322
	jbhi _?L29					*	jhi .L29		|
	and.l #255,d0					*	and.l #255,%d0	|, tmp323
	add.l d0,d0					*	add.l %d0,%d0	| tmp326
	move.w (a5,d0.l),d0				*	move.w (%a5,%d0.l),%d0	|, tmp327
	jmp 2(pc,d0.w)					*	jmp %pc@(2,%d0:w)	| tmp327
...
_?L38:							*.L38:
							*| main.c:732:         g_information_mode = 1;
	moveq #1,d2					*	moveq #1,%d2	|,
	move.l d2,_g_information_mode			*	move.l %d2,g_information_mode	|, g_information_mode
	jbra _?L27					*	jra .L27		|
_?L35:							*.L35:
							*| main.c:734:         g_centering_mode = 1;
	moveq #1,d0					*	moveq #1,%d0	|,
	move.l d0,_g_centering_mode			*	move.l %d0,g_centering_mode	|, g_centering_mode
	jbra _?L27					*	jra .L27		|
_?L37:							*.L37:
							*| main.c:736:         g_wait_mode = 1;
	moveq #1,d1					*	moveq #1,%d1	|,
	move.l d1,_g_wait_mode				*	move.l %d1,g_wait_mode	|, g_wait_mode
	jbra _?L27					*	jra .L27		|
_?L36:							*.L36:
							*| main.c:738:         g_memory_cache_mode = 1;
	moveq #1,d2					*	moveq #1,%d2	|,
	move.l d2,_g_memory_cache_mode			*	move.l %d2,g_memory_cache_mode	|, g_memory_cache_mode
	jbra _?L27					*	jra .L27		|
_?L32:							*.L32:
							*| main.c:740:         g_high_memory_mode = 1;
	moveq #1,d0					*	moveq #1,%d0	|,
	move.l d0,_g_high_memory_mode			*	move.l %d0,g_high_memory_mode	|, g_high_memory_mode
	jbra _?L27					*	jra .L27		|
_?L30:							*.L30:
							*| main.c:742:         g_brightness = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:742:         g_brightness = atoi(argv[i]+2);
	move.l d0,_g_brightness				*	move.l %d0,g_brightness	| tmp987, g_brightness
	jbra _?L27					*	jra .L27		|
_?L33:							*.L33:
							*| main.c:744:         g_screen_mode = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:744:         g_screen_mode = atoi(argv[i]+2);
	move.l d0,_g_screen_mode			*	move.l %d0,g_screen_mode	| tmp988, g_screen_mode
	jbra _?L27					*	jra .L27		|
_?L39:							*.L39:
							*| main.c:746:         g_image_index_limit = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:746:         g_image_index_limit = atoi(argv[i]+2);
	move.l d0,_g_image_index_limit			*	move.l %d0,g_image_index_limit	| tmp989, g_image_index_limit
	jbra _?L27					*	jra .L27		|
_?L34:							*.L34:
							*| main.c:749:         strcpy(opt,argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	pea (_opt?0)					*	pea (opt.0)		|
	jbsr _strcpy					*	jsr strcpy		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:750:         char* c = strchr(opt,',');
	pea 44.w					*	pea 44.w		|
	pea (_opt?0)					*	pea (opt.0)		|
	jbsr _strchr					*	jsr strchr		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,a3					*	move.l %d0,%a3	| tmp990, c
							*| main.c:751:         if (c != NULL) {
	tst.l d0					*	tst.l %d0	| c
	jbeq _?L42					*	jeq .L42		|
							*| main.c:752:           *c = '\0';
	clr.b (a3)+					*	clr.b (%a3)+	| *c_70
							*| main.c:753:           g_start_x = atoi(opt);
	pea (_opt?0)					*	pea (opt.0)		|
	jbsr _atoi					*	jsr atoi		|
							*| main.c:753:           g_start_x = atoi(opt);
	move.l d0,_g_start_x				*	move.l %d0,g_start_x	| tmp991, g_start_x
							*| main.c:754:           g_start_y = atoi(c+1);
	move.l a3,(sp)					*	move.l %a3,(%sp)	| tmp338,
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:754:           g_start_y = atoi(c+1);
	move.l d0,_g_start_y				*	move.l %d0,g_start_y	| tmp992, g_start_y
	jbra _?L27					*	jra .L27		|
_?L42:							*.L42:
							*| main.c:756:           g_start_x = atoi(opt);
	pea (_opt?0)					*	pea (opt.0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:756:           g_start_x = atoi(opt);
	move.l d0,_g_start_x				*	move.l %d0,g_start_x	| tmp993, g_start_x
	jbra _?L27					*	jra .L27		|
_?L40:							*.L40:
							*| main.c:759:         g_buffer_memory_size_factor = atoi(argv[i]+2);
	pea 2(a0)					*	pea 2(%a0)		|
	jbsr _atoi					*	jsr atoi		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:759:         g_buffer_memory_size_factor = atoi(argv[i]+2);
	move.l d0,_g_buffer_memory_size_factor		*	move.l %d0,g_buffer_memory_size_factor	| _19, g_buffer_memory_size_factor
							*| main.c:760:         if (g_buffer_memory_size_factor > 16) {
	moveq #16,d1					*	moveq #16,%d1	|,
	cmp.l d0,d1					*	cmp.l %d0,%d1	| _19,
	jbge _?L27					*	jge .L27		|
							*| main.c:761:           printf("error: too large memory factor.\n");
	pea _?LC17					*	pea .LC17		|
_?L166:							*.L166:
							*| main.c:777:     printf("error: no input file.\n");
	jbsr _puts					*	jsr puts		|
	...
	jbra _?L26					*	jra .L26		|
_?L29:							*.L29:
							*| main.c:764:       } else if (argv[i][1] == 'h') {
	cmp.b #104,d1					*	cmp.b #104,%d1	|, _6
	jbne _?L43					*	jne .L43		|
							*| main.c:765:         show_help_message();
	jbsr _show_help_message				*	jsr show_help_message		|
_?L44:							*.L44:
							*| main.c:766:         return 0;
...
_?L24:							*.L24:
	...
	rts						*	rts
_?L43:							*.L43:
							*| main.c:768:         printf("error: unknown option (%s).\n",argv[i]);
...
_?L116:							*.L116:
	...
	jbra _?L27					*	jra .L27		|
_?L46:							*.L46:
							*| main.c:796:   g_input_buffer_size = 65536 * g_buffer_memory_size_factor;
	...
	move.l d6,_g_input_buffer_size			*	move.l %d6,g_input_buffer_size	| _24, g_input_buffer_size
							*| main.c:799:   g_output_buffer_size = 131072 * g_buffer_memory_size_factor;
	lsl.w #1,d0					*	lsl.w #1,%d0	|, tmp347
	swap d0						*	swap %d0	| tmp347
	clr.w d0					*	clr.w %d0	| tmp347
							*| main.c:799:   g_output_buffer_size = 131072 * g_buffer_memory_size_factor;
	move.l d0,_g_output_buffer_size			*	move.l %d0,g_output_buffer_size	| tmp347, g_output_buffer_size
							*| main.c:802:   g_actual_width = g_screen_mode == 3 ? ACTUAL_WIDTH_EX : ACTUAL_WIDTH;
...
_?L47:							*.L47:
	...
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
							*| main.c:608:     if (FILES(&inf,gif_file_name,0x23) != 0) {
	tst.l d0					*	tst.l %d0	| tmp995
	jbeq _?L49					*	jeq .L49		|
							*| main.c:609:       printf("error: file read error (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC20					*	pea .LC20		|
_?L167:							*.L167:
							*| main.c:616:       printf("error: cannot open input file (%s).\n", gif_file_name);
	jbsr _printf					*	jsr printf		|
_?L158:							*.L158:
							*| main.c:686:       free__(file_buffer_ptr, g_high_memory_mode);
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:690:     return 0;
	jbra _?L44					*	jra .L44		|
_?L49:							*.L49:
							*| main.c:614:     fp = fopen(gif_file_name, "rb");
	pea _?LC21					*	pea .LC21		|
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	jbsr _fopen					*	jsr fopen		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d3					*	move.l %d0,%d3	| tmp996, fp
							*| main.c:615:     if (fp == NULL) {
	jbne _?L51					*	jne .L51		|
							*| main.c:616:       printf("error: cannot open input file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC22					*	pea .LC22		|
...
_?L51:							*.L51:
							*| main.c:621:     file_buffer_ptr = malloc__(file_buffer_size, g_high_memory_mode);
	...
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d4					*	move.l %d0,%d4	| tmp997, file_buffer_ptr
	lea _fclose,a5					*	lea fclose,%a5	|, tmp967
							*| main.c:622:     if (file_buffer_ptr == NULL) {
	jbne _?L52					*	jne .L52		|
							*| main.c:623:       printf("error: cannot allocate memory for file buffer.\n");
	pea _?LC23					*	pea .LC23		|
	jbsr _puts					*	jsr puts		|
							*| main.c:624:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	jbsr (a5)					*	jsr (%a5)		| tmp967
_?L157:							*.L157:
							*| main.c:841:   C_FNKMOD(func_key_display_mode);
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:843:   return 0;
	jbra _?L44					*	jra .L44		|
_?L52:							*.L52:
							*| main.c:629:     read_size = fread(file_buffer_ptr, 1, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	pea 1.w						*	pea 1.w		|
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:630:     if (read_size <= 0) {
	tst.l d0					*	tst.l %d0	| tmp998
	jbgt _?L53					*	jgt .L53		|
							*| main.c:631:       printf("error: file read error.\n");
	pea _?LC24					*	pea .LC24		|
	jbsr _puts					*	jsr puts		|
							*| main.c:632:       fclose(fp);
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
_?L156:							*.L156:
							*| main.c:680:       fclose(fp);
	jbsr (a5)					*	jsr (%a5)		| tmp967
							*| main.c:686:       free__(file_buffer_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
...
_?L53:							*.L53:
							*| main.c:638:     memcpy_buffer(header.signature, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	lea (-114,a6),a4				*	lea (-114,%fp),%a4	|,, tmp360
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea (-98,a6),a3					*	lea (-98,%fp),%a3	|,, tmp361
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp361,
	lea _memcpy_buffer,a0				*	lea memcpy_buffer,%a0	|, tmp362
	move.l a0,-138(a6)				*	move.l %a0,-138(%fp)	|,
	jbsr (a0)					*	jsr (%a0)		| tmp362
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:639:     header.signature[3] = '\0';
	clr.b -95(a6)					*	clr.b -95(%fp)	| MEM[(struct  *)_410].signature[3]
							*| main.c:640:     if (strcmp(header.signature, "GIF") != 0 ) {
	pea _?LC25					*	pea .LC25		|
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp361,
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	lea _printf,a3					*	lea printf,%a3	|, tmp969
							*| main.c:640:     if (strcmp(header.signature, "GIF") != 0 ) {
	move.l -138(a6),a0				*	move.l -138(%fp),%a0	|,
	tst.l d0					*	tst.l %d0	| tmp999
	jbeq _?L54					*	jeq .L54		|
							*| main.c:641:       printf("error: signature error. not a GIF file (%s).\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC26					*	pea .LC26		|
_?L155:							*.L155:
							*| main.c:676:     printf("  aspect ratio: %d\n", header.aspect_ratio);
	jbsr (a3)					*	jsr (%a3)		| tmp969
							*| main.c:680:       fclose(fp);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l d3,(sp)					*	move.l %d3,(%sp)	| fp,
	jbra _?L156					*	jra .L156		|
_?L54:							*.L54:
							*| main.c:648:     memcpy_buffer(header.version, file_buffer_ptr, &file_buffer_ofs, file_buffer_size, 3, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	pea 3.w						*	pea 3.w		|
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	moveq #-94,d7					*	moveq #-94,%d7	|, tmp371
	add.l a6,d7					*	add.l %fp,%d7	|, tmp371
	move.l d7,-(sp)					*	move.l %d7,-(%sp)	| tmp371,
	jbsr (a0)					*	jsr (%a0)		| tmp362
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
							*| main.c:649:     header.version[3] = '\0';
	clr.b -91(a6)					*	clr.b -91(%fp)	| MEM[(struct  *)_410].version[3]
							*| main.c:650:     if (strcmp(header.version, "89a") != 0 ) {
	pea _?LC27					*	pea .LC27		|
	move.l d7,-(sp)					*	move.l %d7,-(%sp)	| tmp371,
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:650:     if (strcmp(header.version, "89a") != 0 ) {
	tst.l d0					*	tst.l %d0	| tmp1000
	jbeq _?L55					*	jeq .L55		|
							*| main.c:651:       printf("error: unsupported GIF version (%s).\n", header.version);
	move.l d7,-(sp)					*	move.l %d7,-(%sp)	| tmp371,
	pea _?LC28					*	pea .LC28		|
	jbra _?L155					*	jra .L155		|
_?L55:							*.L55:
							*| main.c:658:     header.screen_width   = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea _get_ushort_buffer,a0			*	lea get_ushort_buffer,%a0	|, tmp383
	move.l a0,-138(a6)				*	move.l %a0,-138(%fp)	|,
	jbsr (a0)					*	jsr (%a0)		| tmp383
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:658:     header.screen_width   = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,-90(a6)				*	move.w %d0,-90(%fp)	| tmp1001, MEM[(struct  *)_410].screen_width
							*| main.c:659:     header.screen_height  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:659:     header.screen_height  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,-88(a6)				*	move.w %d0,-88(%fp)	| tmp1002, MEM[(struct  *)_410].screen_height
							*| main.c:660:     header.flag_code      = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:660:     header.flag_code      = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-86(a6)				*	move.b %d0,-86(%fp)	| tmp1003, MEM[(struct  *)_410].flag_code
							*| main.c:661:     header.bg_color_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:661:     header.bg_color_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-85(a6)				*	move.b %d0,-85(%fp)	| tmp1004, MEM[(struct  *)_410].bg_color_index
							*| main.c:662:     header.aspect_ratio   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| _24,
	move.l a4,-(sp)					*	move.l %a4,-(%sp)	| tmp360,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:662:     header.aspect_ratio   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-84(a6)				*	move.b %d0,-84(%fp)	| tmp1005, MEM[(struct  *)_410].aspect_ratio
							*| main.c:665:     printf("     file name: %s\n", gif_file_name);
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	| gif_file_name,
	pea _?LC29					*	pea .LC29		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
							*| main.c:666:     printf("     file size: %d\n", inf.filelen);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l -28(a6),(sp)				*	move.l -28(%fp),(%sp)	| MEM[(struct FILBUF *)_277].filelen,
	pea _?LC30					*	pea .LC30		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:667:     printf("     file time: %04d-%02d-%02d %02d:%02d:%02d\n", 1980+(inf.date>>9), (inf.date>>5)&0xf, inf.date&0x1f, inf.time>>11, (inf.time>>5)&0x3f, inf.time&0x1f);
	...
	move.w d1,d2					*	move.w %d1,%d2	| _111, tmp396
	lsr.w #5,d2					*	lsr.w #5,%d2	|, tmp396
	moveq #63,d5					*	moveq #63,%d5	|,
	and.l d2,d5					*	and.l %d2,%d5	| tmp396,
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	|,
	moveq #11,d2					*	moveq #11,%d2	|,
	lsr.w d2,d1					*	lsr.w %d2,%d1	|, tmp399
	moveq #31,d5					*	moveq #31,%d5	|,
	and.l d1,d5					*	and.l %d1,%d5	| tmp399,
	move.l d5,-(sp)					*	move.l %d5,-(%sp)	|,
	moveq #31,d1					*	moveq #31,%d1	|,
	and.l d0,d1					*	and.l %d0,%d1	| _119,
	move.l d1,-(sp)					*	move.l %d1,-(%sp)	|,
	move.w d0,d1					*	move.w %d0,%d1	| _119, tmp403
	lsr.w #5,d1					*	lsr.w #5,%d1	|, tmp403
	moveq #15,d2					*	moveq #15,%d2	|,
	and.l d1,d2					*	and.l %d1,%d2	| tmp403,
	move.l d2,-(sp)					*	move.l %d2,-(%sp)	|,
							*| main.c:667:     printf("     file time: %04d-%02d-%02d %02d:%02d:%02d\n", 1980+(inf.date>>9), (inf.date>>5)&0xf, inf.date&0x1f, inf.time>>11, (inf.time>>5)&0x3f, inf.time&0x1f);
	moveq #9,d5					*	moveq #9,%d5	|,
	lsr.w d5,d0					*	lsr.w %d5,%d0	|, tmp406
							*| main.c:667:     printf("     file time: %04d-%02d-%02d %02d:%02d:%02d\n", 1980+(inf.date>>9), (inf.date>>5)&0xf, inf.date&0x1f, inf.time>>11, (inf.time>>5)&0x3f, inf.time&0x1f);
	add.w #1980,d0					*	add.w #1980,%d0	|, tmp407
	move.l d0,d1					*	move.l %d0,%d1	|,
	and.l #4095,d1					*	and.l #4095,%d1	|,
	move.l d1,-(sp)					*	move.l %d1,-(%sp)	|,
	pea _?LC31					*	pea .LC31		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
							*| main.c:668:     printf("   GIF version: %s\n", header.version);
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
	move.l d7,(sp)					*	move.l %d7,(%sp)	| tmp371,
	pea _?LC32					*	pea .LC32		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:669:     printf("  screen width: %d\n", header.screen_width);
	moveq #0,d0					*	moveq #0,%d0	| MEM[(struct  *)_410].screen_width
	move.w -90(a6),d0				*	move.w -90(%fp),%d0	| MEM[(struct  *)_410].screen_width, MEM[(struct  *)_410].screen_width
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| MEM[(struct  *)_410].screen_width,
	pea _?LC33					*	pea .LC33		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:670:     printf(" screen height: %d\n", header.screen_height);
	moveq #0,d0					*	moveq #0,%d0	| MEM[(struct  *)_410].screen_height
	move.w -88(a6),d0				*	move.w -88(%fp),%d0	| MEM[(struct  *)_410].screen_height, MEM[(struct  *)_410].screen_height
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| MEM[(struct  *)_410].screen_height,
	pea _?LC34					*	pea .LC34		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:671:     printf("     bit depth: %d\n", 1 + read_bits(header.flag_code,4,3));
	moveq #0,d0					*	moveq #0,%d0	| MEM[(struct  *)_410].flag_code
	move.b -86(a6),d0				*	move.b -86(%fp),%d0	| MEM[(struct  *)_410].flag_code, MEM[(struct  *)_410].flag_code
							*| main.c:69:   return ( byte_data >> bit_ofs ) & ( 0xff >> ( 8 - bit_len ));
	asr.l #4,d0					*	asr.l #4,%d0	|, tmp418
							*| main.c:671:     printf("     bit depth: %d\n", 1 + read_bits(header.flag_code,4,3));
	moveq #7,d2					*	moveq #7,%d2	|,
	and.l d2,d0					*	and.l %d2,%d0	|, tmp422
	move.l d0,a0					*	move.l %d0,%a0	| tmp422,
	pea 1(a0)					*	pea 1(%a0)		|
	pea _?LC35					*	pea .LC35		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:672:     printf(" GCT available: %s\n", read_bits(header.flag_code,7,1) ? "yes" : "no");
	move.l #_?LC16,d0				*	move.l #.LC16,%d0	|, iftmp.54_142
	tst.b -86(a6)					*	tst.b -86(%fp)	| MEM[(struct  *)_410].flag_code
	jbpl _?L56					*	jpl .L56		|
...
_?L56:							*.L56:
	...
	move.b -86(a6),d1				*	move.b -86(%fp),%d1	| MEM[(struct  *)_410].flag_code, _144
							*| main.c:673:     printf("      GCT sort: %s\n", read_bits(header.flag_code,7,1) && read_bits(header.flag_code,3,1) ? "yes" : "no");
...
_?L57:							*.L57:
	...
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:674:     printf("      GCT size: %d\n", 3 * read_bits(header.flag_code,0,3));
	move.b -86(a6),d0				*	move.b -86(%fp),%d0	| MEM[(struct  *)_410].flag_code, tmp431
	and.b #7,d0					*	and.b #7,%d0	|, tmp431
	moveq #7,d1					*	moveq #7,%d1	|,
	and.l d1,d0					*	and.l %d1,%d0	|, tmp433
	move.l d0,a0					*	move.l %d0,%a0	| tmp433, tmp435
	add.l d0,a0					*	add.l %d0,%a0	| tmp433, tmp435
	pea (a0,d0.l)					*	pea (%a0,%d0.l)		|
	pea _?LC38					*	pea .LC38		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:675:     printf("BG color index: %d\n", header.bg_color_index);
	moveq #0,d0					*	moveq #0,%d0	| MEM[(struct  *)_410].bg_color_index
	move.b -85(a6),d0				*	move.b -85(%fp),%d0	| MEM[(struct  *)_410].bg_color_index, MEM[(struct  *)_410].bg_color_index
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| MEM[(struct  *)_410].bg_color_index,
	pea _?LC39					*	pea .LC39		|
	jbsr (a3)					*	jsr (%a3)		| tmp969
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:676:     printf("  aspect ratio: %d\n", header.aspect_ratio);
	moveq #0,d0					*	moveq #0,%d0	| MEM[(struct  *)_410].aspect_ratio
	move.b -84(a6),d0				*	move.b -84(%fp),%d0	| MEM[(struct  *)_410].aspect_ratio, MEM[(struct  *)_410].aspect_ratio
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| MEM[(struct  *)_410].aspect_ratio,
	pea _?LC40					*	pea .LC40		|
	jbra _?L155					*	jra .L155		|
_?L48:							*.L48:
	move.l _g_brightness,d3				*	move.l g_brightness,%d3	| g_brightness, g_brightness
	lsl.l #5,d3					*	lsl.l #5,%d3	|, _603
	...
	lea _g_rgb555_r,a5				*	lea g_rgb555_r,%a5	|, tmp451
							*| main.c:79:     g_rgb555_g[i] = (unsigned short)((c << 11) + 1);
	lea _g_rgb555_g,a4				*	lea g_rgb555_g,%a4	|, tmp453
							*| main.c:80:     g_rgb555_b[i] = (unsigned short)((c <<  1) + 1);
	lea _g_rgb555_b,a3				*	lea g_rgb555_b,%a3	|, tmp455
_?L59:							*.L59:
							*| main.c:77:     unsigned int c = (int)(i * 32 * g_brightness / 100) >> 8;
	pea 100.w					*	pea 100.w		|
	...
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:77:     unsigned int c = (int)(i * 32 * g_brightness / 100) >> 8;
	asr.l #8,d0					*	asr.l #8,%d0	|, tmp450
							*| main.c:78:     g_rgb555_r[i] = (unsigned short)((c <<  6) + 1);
	move.w d0,d1					*	move.w %d0,%d1	|, tmp452
	lsl.w #6,d1					*	lsl.w #6,%d1	|, tmp452
							*| main.c:78:     g_rgb555_r[i] = (unsigned short)((c <<  6) + 1);
	addq.w #1,d1					*	addq.w #1,%d1	|, tmp452
	move.w d1,(a5,d4.l)				*	move.w %d1,(%a5,%d4.l)	| tmp452, MEM[(short unsigned int *)&g_rgb555_r + ivtmp.145_582 * 1]
							*| main.c:79:     g_rgb555_g[i] = (unsigned short)((c << 11) + 1);
	move.w d0,d1					*	move.w %d0,%d1	|, tmp454
	moveq #11,d2					*	moveq #11,%d2	|,
	lsl.w d2,d1					*	lsl.w %d2,%d1	|, tmp454
							*| main.c:79:     g_rgb555_g[i] = (unsigned short)((c << 11) + 1);
	addq.w #1,d1					*	addq.w #1,%d1	|, tmp454
	move.w d1,(a4,d4.l)				*	move.w %d1,(%a4,%d4.l)	| tmp454, MEM[(short unsigned int *)&g_rgb555_g + ivtmp.145_582 * 1]
							*| main.c:80:     g_rgb555_b[i] = (unsigned short)((c <<  1) + 1);
	add.w d0,d0					*	add.w %d0,%d0	| tmp450, tmp456
							*| main.c:80:     g_rgb555_b[i] = (unsigned short)((c <<  1) + 1);
	addq.w #1,d0					*	addq.w #1,%d0	|, tmp456
	move.w d0,(a3,d4.l)				*	move.w %d0,(%a3,%d4.l)	| tmp456, MEM[(short unsigned int *)&g_rgb555_b + ivtmp.145_582 * 1]
							*| main.c:76:   for (int i = 0; i < 256; i++) {
	add.l d3,d6					*	add.l %d3,%d6	| _603, ivtmp.138
	addq.l #2,d4					*	addq.l #2,%d4	|, ivtmp.145
	cmp.l #512,d4					*	cmp.l #512,%d4	|, ivtmp.145
	jbne _?L59					*	jne .L59		|
							*| main.c:814:   if (g_clear_screen) {
...
_?L61:							*.L61:
	...
	jbsr _fopen					*	jsr fopen		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d3					*	move.l %d0,%d3	| tmp1009, fp
...
_?L63:							*.L63:
	...
	jbsr _malloc__					*	jsr malloc__		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,d4					*	move.l %d0,%d4	| tmp1010, file_buffer_ptr
...
_?L65:							*.L65:
	...
	move.l #_memcpy_buffer,d7			*	move.l #memcpy_buffer,%d7	|, tmp478
	move.l d7,a0					*	move.l %d7,%a0	| tmp478,
	jbsr (a0)					*	jsr (%a0)		|
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
...
_?L66:							*.L66:
	...
	pea -118(a6)					*	pea -118(%fp)		|
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	moveq #-110,d5					*	moveq #-110,%d5	|, tmp487
	pea (a6,d5.l)					*	pea (%fp,%d5.l)		|
	move.l d7,a2					*	move.l %d7,%a2	| tmp478,
	jbsr (a2)					*	jsr (%a2)		|
	lea (24,sp),sp					*	lea (24,%sp),%sp	|,
	...
	jbsr _strcmp					*	jsr strcmp		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,-130(a6)				*	move.l %d0,-130(%fp)	| tmp1013, %sfp
							*| main.c:214:     if (strcmp(header.version,"89a") != 0 ) {
	jbeq _?L67					*	jeq .L67		|
							*| main.c:215:       printf("error: unsupported GIF version (%s).\n", header.version);
	pea (a6,d5.l)					*	pea (%fp,%d5.l)		|
...
_?L67:							*.L67:
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:222:     header.screen_width   = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,-106(a6)				*	move.w %d0,-106(%fp)	| tmp1014, MEM[(struct  *)_279].screen_width
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:223:     header.screen_height  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,-104(a6)				*	move.w %d0,-104(%fp)	| tmp1015, MEM[(struct  *)_279].screen_height
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:224:     header.flag_code      = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-102(a6)				*	move.b %d0,-102(%fp)	| tmp1016, MEM[(struct  *)_279].flag_code
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:225:     header.bg_color_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-101(a6)				*	move.b %d0,-101(%fp)	| tmp1017, MEM[(struct  *)_279].bg_color_index
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:226:     header.aspect_ratio   = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-100(a6)				*	move.b %d0,-100(%fp)	| tmp1018, MEM[(struct  *)_279].aspect_ratio
...
_?L72:							*.L72:
	...
	jbsr _malloc__					*	jsr malloc__		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,-134(a6)				*	move.l %d0,-134(%fp)	| tmp1019, %sfp
...
_?L70:							*.L70:
	...
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	lea (-118,a6),a3				*	lea (-118,%fp),%a3	|,, tmp534
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp534,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea _get_uchar_buffer,a4			*	lea get_uchar_buffer,%a4	|, tmp535
	jbsr (a4)					*	jsr (%a4)		| tmp535
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	...
	move.l d5,d7					*	move.l %d5,%d7	| image_frame_index, tmp538
	lsl.l #4,d7					*	lsl.l #4,%d7	|, tmp538
	move.l d7,d0					*	move.l %d7,%d0	| tmp538, tmp539
	add.l d5,d0					*	add.l %d5,%d0	| image_frame_index, tmp539
	add.l d0,d0					*	add.l %d0,%d0	| tmp539, tmp540
	add.l d0,d0					*	add.l %d0,%d0	| tmp540, tmp541
	add.l d5,d0					*	add.l %d5,%d0	| image_frame_index, tmp542
	lsl.l #3,d0					*	lsl.l #3,%d0	|, tmp543
	move.l d0,a5					*	move.l %d0,%a5	| tmp543, tmp544
	add.l #_g_image_frames,a5			*	add.l #g_image_frames,%a5	|, tmp544
							*| main.c:273:         image_frame->bg_color_index = header.bg_color_index;
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:286:         image_block->left_position = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,22(a5)				*	move.w %d0,22(%a5)	| tmp1024, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.left_position
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:287:         image_block->top_position  = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,24(a5)				*	move.w %d0,24(%a5)	| tmp1025, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.top_position
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:288:         image_block->width         = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,26(a5)				*	move.w %d0,26(%a5)	| tmp1026, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.width
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:289:         image_block->height        = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,28(a5)				*	move.w %d0,28(%a5)	| tmp1027, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][image_frame_index_217].image_block.height
	...
	move.w _g_image_frames+26,d2			*	move.w g_image_frames+26,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
							*| main.c:296:               g_start_x = (384 - image_block->width) / 2;
	move.l #384,d1					*	move.l #384,%d1	|, tmp629
	sub.l d2,d1					*	sub.l %d2,%d1	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, tmp629
							*| main.c:296:               g_start_x = (384 - image_block->width) / 2;
	move.l d1,d1					*	move.l %d1,%d1	| tmp629, tmp631
	jbpl _?L170					*	jpl .L170		|
	addq.l #1,d1					*	addq.l #1,%d1	| tmp631
_?L170:							*.L170:
	asr.l #1,d1					*	asr.l #1,%d1	|, tmp632
							*| main.c:296:               g_start_x = (384 - image_block->width) / 2;
	move.l d1,_g_start_x				*	move.l %d1,g_start_x	| tmp632, g_start_x
	...
	move.w _g_image_frames+28,d2			*	move.w g_image_frames+28,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height
							*| main.c:297:               g_start_y = (256 - image_block->height) / 2;
	move.l #256,d1					*	move.l #256,%d1	|, tmp635
_?L159:							*.L159:
							*| main.c:303:               g_start_y = (512 - image_block->height) / 2;
	sub.l d2,d1					*	sub.l %d2,%d1	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height, tmp660
							*| main.c:303:               g_start_y = (512 - image_block->height) / 2;
	move.l d1,d1					*	move.l %d1,%d1	| tmp660, tmp662
	jbpl _?L171					*	jpl .L171		|
	addq.l #1,d1					*	addq.l #1,%d1	| tmp662
_?L171:							*.L171:
	asr.l #1,d1					*	asr.l #1,%d1	|, tmp663
							*| main.c:303:               g_start_y = (512 - image_block->height) / 2;
	move.l d1,_g_start_y				*	move.l %d1,g_start_y	| tmp663, g_start_y
_?L79:							*.L79:
...
_?L85:							*.L85:
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:322:         image_block->lzw_min_code_size = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.l d5,d7					*	move.l %d5,%d7	| image_frame_index, tmp670
	lsl.l #4,d7					*	lsl.l #4,%d7	|, tmp670
	move.l d7,d1					*	move.l %d7,%d1	| tmp670, tmp671
	add.l d5,d1					*	add.l %d5,%d1	| image_frame_index, tmp671
	add.l d1,d1					*	add.l %d1,%d1	| tmp671, tmp672
	add.l d1,d1					*	add.l %d1,%d1	| tmp672, tmp673
	add.l d5,d1					*	add.l %d5,%d1	| image_frame_index, tmp674
	lsl.l #3,d1					*	lsl.l #3,%d1	|, tmp675
	move.l d1,a0					*	move.l %d1,%a0	| tmp675, tmp676
	add.l #_g_image_frames,a0			*	add.l #g_image_frames,%a0	|, tmp676
	...
	jbsr _malloc__					*	jsr malloc__		|
	addq.l #8,sp					*	addq.l #8,%sp	|,
	move.l d0,a3					*	move.l %d0,%a3	| tmp1030, sub_block_data2
...
_?L83:							*.L83:
	...
	move.l d0,a4					*	move.l %d0,%a4	| tmp1035, decode_buffer_ofs
	...
	add.l d5,d7					*	add.l %d5,%d7	| image_frame_index, tmp767
	add.l d7,d7					*	add.l %d7,%d7	| tmp767, tmp768
	add.l d7,d7					*	add.l %d7,%d7	| tmp768, tmp769
	move.l d7,d0					*	move.l %d7,%d0	| tmp769, tmp770
	add.l d5,d0					*	add.l %d5,%d0	| image_frame_index, tmp770
	lsl.l #3,d0					*	lsl.l #3,%d0	|, tmp771
	move.l d0,a0					*	move.l %d0,%a0	| tmp771, tmp772
	add.l #_g_image_frames,a0			*	add.l #g_image_frames,%a0	|, tmp772
	lea (548,a0),a5					*	lea (548,%a0),%a5	|, tmp772, tmp773
	...
	move.l a0,-(sp)					*	move.l %a0,-(%sp)	| tmp772,
	jbsr _output_image				*	jsr output_image		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
...
_?L68:							*.L68:
	...
	moveq #7,d1					*	moveq #7,%d1	|,
	and.l d1,d0					*	and.l %d1,%d0	|, tmp512
	addq.l #1,d0					*	addq.l #1,%d0	|, tmp513
	moveq #2,d1					*	moveq #2,%d1	|, tmp515
	lsl.l d0,d1					*	lsl.l %d0,%d1	| tmp513, tmp514
	add.l a0,d1					*	add.l %a0,%d1	| ivtmp.132, tmp514
	move.l d1,-134(a6)				*	move.l %d1,-134(%fp)	| tmp514, %sfp
_?L71:							*.L71:
	...
	jbsr _get_uchar_buffer				*	jsr get_uchar_buffer		|
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.b d0,d7					*	move.b %d0,%d7	| tmp1020, r
	...
	jbsr _get_uchar_buffer				*	jsr get_uchar_buffer		|
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.b d0,d5					*	move.b %d0,%d5	| tmp1021, g
	...
	add.l d7,d7					*	add.l %d7,%d7	| r, tmp524
							*| main.c:235:         g_global_color_table[i] = g_rgb555_r[ r ] | g_rgb555_g[ g ] | g_rgb555_b[ b ];
	and.l #255,d5					*	and.l #255,%d5	|, g
	add.l d5,d5					*	add.l %d5,%d5	| g, tmp527
							*| main.c:235:         g_global_color_table[i] = g_rgb555_r[ r ] | g_rgb555_g[ g ] | g_rgb555_b[ b ];
	move.w (a5,d7.l),d1				*	move.w (%a5,%d7.l),%d1	| g_rgb555_r[_202], tmp528
	or.w (a4,d5.l),d1				*	or.w (%a4,%d5.l),%d1	| g_rgb555_g[_204], tmp528
							*| main.c:235:         g_global_color_table[i] = g_rgb555_r[ r ] | g_rgb555_g[ g ] | g_rgb555_b[ b ];
	and.l #255,d0					*	and.l #255,%d0	|, b
	add.l d0,d0					*	add.l %d0,%d0	| b, tmp531
							*| main.c:235:         g_global_color_table[i] = g_rgb555_r[ r ] | g_rgb555_g[ g ] | g_rgb555_b[ b ];
	move.l -138(a6),a0				*	move.l -138(%fp),%a0	|,
	or.w (a3,d0.l),d1				*	or.w (%a3,%d0.l),%d1	| g_rgb555_b[_207], tmp528
	move.w d1,(a0)+					*	move.w %d1,(%a0)+	| tmp528, MEM[(short unsigned int *)_593]
...
_?L78:							*.L78:
	...
	move.l d1,a0					*	move.l %d1,%a0	| g_screen_mode.68_227, tmp639
	subq.l #1,a0					*	subq.l #1,%a0	|, tmp639
							*| main.c:298:             } else if (g_screen_mode == 1 || g_screen_mode == 2) {
	moveq #1,d2					*	moveq #1,%d2	|,
	cmp.l a0,d2					*	cmp.l %a0,%d2	| tmp639,
	jbcs _?L80					*	jcs .L80		|
							*| main.c:299:               g_start_x = (512 - image_block->width) / 2;
	moveq #0,d2					*	moveq #0,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
	move.w _g_image_frames+26,d2			*	move.w g_image_frames+26,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
							*| main.c:299:               g_start_x = (512 - image_block->width) / 2;
	move.l #512,d1					*	move.l #512,%d1	|, tmp642
_?L160:							*.L160:
							*| main.c:302:               g_start_x = (768 - image_block->width) / 2;
	sub.l d2,d1					*	sub.l %d2,%d1	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, tmp654
							*| main.c:302:               g_start_x = (768 - image_block->width) / 2;
	move.l d1,d1					*	move.l %d1,%d1	| tmp654, tmp656
	jbpl _?L172					*	jpl .L172		|
	addq.l #1,d1					*	addq.l #1,%d1	| tmp656
_?L172:							*.L172:
	asr.l #1,d1					*	asr.l #1,%d1	|, tmp657
							*| main.c:302:               g_start_x = (768 - image_block->width) / 2;
	move.l d1,_g_start_x				*	move.l %d1,g_start_x	| tmp657, g_start_x
							*| main.c:303:               g_start_y = (512 - image_block->height) / 2;
	moveq #0,d2					*	moveq #0,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height
	move.w _g_image_frames+28,d2			*	move.w g_image_frames+28,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.height
							*| main.c:303:               g_start_y = (512 - image_block->height) / 2;
	move.l #512,d1					*	move.l #512,%d1	|, tmp660
...
_?L80:							*.L80:
	...
	move.w _g_image_frames+26,d2			*	move.w g_image_frames+26,%d2	| MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_IMAGE_BLOCK *)&g_image_frames][0].image_block.width
							*| main.c:302:               g_start_x = (768 - image_block->width) / 2;
	move.l #768,d1					*	move.l #768,%d1	|, tmp654
	jbra _?L160					*	jra .L160		|
_?L82:							*.L82:
							*| main.c:312:           int lct_size = 1 << (1 + (read_bits(image_block->flag_code,0,3)));
	moveq #7,d2					*	moveq #7,%d2	|,
	and.l d2,d0					*	and.l %d2,%d0	|, tmp703
	addq.l #1,d0					*	addq.l #1,%d0	|, tmp704
							*| main.c:312:           int lct_size = 1 << (1 + (read_bits(image_block->flag_code,0,3)));
	moveq #1,d1					*	moveq #1,%d1	|, tmp705
	lsl.l d0,d1					*	lsl.l %d0,%d1	| tmp704, tmp705
	move.l d1,-126(a6)				*	move.l %d1,-126(%fp)	| tmp705, %sfp
	add.l d5,d7					*	add.l %d5,%d7	| image_frame_index, tmp708
	add.l d7,d7					*	add.l %d7,%d7	| tmp708, tmp709
	add.l d7,d7					*	add.l %d7,%d7	| tmp709, tmp710
	move.l d7,d0					*	move.l %d7,%d0	| tmp710, tmp711
	add.l d5,d0					*	add.l %d5,%d0	| image_frame_index, tmp711
	lsl.l #3,d0					*	lsl.l #3,%d0	|, tmp712
	move.l d0,a0					*	move.l %d0,%a0	| tmp712, ivtmp.128
	...
	lea (-118,a6),a5				*	lea (-118,%fp),%a5	|,, tmp714
	lea _get_uchar_buffer,a4			*	lea get_uchar_buffer,%a4	|, tmp715
_?L84:							*.L84:
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	move.l d6,-(sp)					*	move.l %d6,-(%sp)	| file_buffer_size,
	move.l a5,-(sp)					*	move.l %a5,-(%sp)	| tmp714,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	move.l a0,-138(a6)				*	move.l %a0,-138(%fp)	|,
	jbsr (a4)					*	jsr (%a4)		| tmp715
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.w d0,a3					*	move.w %d0,%a3	| tmp1031, r
	...
	jbsr (a4)					*	jsr (%a4)		| tmp715
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
	move.b d0,d7					*	move.b %d0,%d7	| tmp1032, g
	...
	add.l d1,a3					*	add.l %d1,%a3	| r, tmp722
							*| main.c:317:             image_frame->local_color_table[i] = g_rgb555_r[ r ] + g_rgb555_g[ g ] + g_rgb555_b[ b ];
	and.l #255,d7					*	and.l #255,%d7	|, g
	add.l d7,d7					*	add.l %d7,%d7	| g, tmp725
							*| main.c:317:             image_frame->local_color_table[i] = g_rgb555_r[ r ] + g_rgb555_g[ g ] + g_rgb555_b[ b ];
	lea _g_rgb555_r,a2				*	lea g_rgb555_r,%a2	|,
	move.w (a3,a2.l),a1				*	move.w (%a3,%a2.l),%a1	| g_rgb555_r[_265], tmp726
	lea _g_rgb555_g,a2				*	lea g_rgb555_g,%a2	|,
	add.w (a2,d7.l),a1				*	add.w (%a2,%d7.l),%a1	| g_rgb555_g[_267], tmp726
							*| main.c:317:             image_frame->local_color_table[i] = g_rgb555_r[ r ] + g_rgb555_g[ g ] + g_rgb555_b[ b ];
	and.l #255,d0					*	and.l #255,%d0	|, b
	add.l d0,d0					*	add.l %d0,%d0	| b, tmp729
							*| main.c:317:             image_frame->local_color_table[i] = g_rgb555_r[ r ] + g_rgb555_g[ g ] + g_rgb555_b[ b ];
	lea _g_rgb555_b,a2				*	lea g_rgb555_b,%a2	|, tmp727
	move.l -138(a6),a0				*	move.l -138(%fp),%a0	|,
	add.w (a2,d0.l),a1				*	add.w (%a2,%d0.l),%a1	| g_rgb555_b[_270], tmp726
	move.w a1,(a0)+					*	move.w %a1,(%a0)+	| tmp726, MEM <short unsigned int> [(struct GIF_IMAGE_FRAME *)_599]
							*| main.c:313:           for (int i = 0; i < lct_size; i++) {
...
_?L87:							*.L87:
	...
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:362:           if ((image_frame->frame_data_ptr = malloc__(size,g_high_memory_mode)) == NULL) {
	move.l d5,d1					*	move.l %d5,%d1	| image_frame_index, tmp740
	lsl.l #4,d1					*	lsl.l #4,%d1	|, tmp740
	add.l d5,d1					*	add.l %d5,%d1	| image_frame_index, tmp741
	add.l d1,d1					*	add.l %d1,%d1	| tmp741, tmp742
	add.l d1,d1					*	add.l %d1,%d1	| tmp742, tmp743
	add.l d5,d1					*	add.l %d5,%d1	| image_frame_index, tmp744
	lsl.l #3,d1					*	lsl.l #3,%d1	|, tmp745
	move.l d1,a5					*	move.l %d1,%a5	| tmp745, tmp746
	add.l #_g_image_frames,a5			*	add.l #g_image_frames,%a5	|, tmp746
...
_?L91:							*.L91:
	...
	move.l _g_memory_cache_mode,d0			*	move.l g_memory_cache_mode,%d0	| g_memory_cache_mode, pretmp_349
							*| main.c:397:         if (image_frame_index >= g_image_index_limit || image_frame_index >= MAX_IMAGE_FRAMES) {
	cmp.l #1023,d5					*	cmp.l #1023,%d5	|, image_frame_index
	jbgt _?L73					*	jgt .L73		|
							*| main.c:401:         if (g_memory_cache_mode && (image_frame_index % 30) == 0) {
	tst.l d0					*	tst.l %d0	| pretmp_349
	jbeq _?L70					*	jeq .L70		|
	pea 30.w					*	pea 30.w		|
...
_?L74:							*.L74:
	...
	lea _g_image_frames,a5				*	lea g_image_frames,%a5	|, tmp819
	move.l d5,d0					*	move.l %d5,%d0	| image_frame_index, tmp821
	lsl.l #4,d0					*	lsl.l #4,%d0	|, tmp821
	add.l d5,d0					*	add.l %d5,%d0	| image_frame_index, tmp822
	add.l d0,d0					*	add.l %d0,%d0	| tmp822, tmp823
	add.l d0,d0					*	add.l %d0,%d0	| tmp823, tmp824
	move.l d0,d7					*	move.l %d0,%d7	| tmp824, tmp825
	add.l d5,d7					*	add.l %d5,%d7	| image_frame_index, tmp825
	lsl.l #3,d7					*	lsl.l #3,%d7	|, tmp826
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:421:           graphic_ctrl_ext->block_size        = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	add.l d7,a5					*	add.l %d7,%a5	| tmp826, tmp837
	move.b d0,14(a5)				*	move.b %d0,14(%a5)	| tmp1039, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.block_size
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:422:           graphic_ctrl_ext->flag_code         = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,15(a5)				*	move.b %d0,15(%a5)	| tmp1040, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.flag_code
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:423:           graphic_ctrl_ext->delay_time        = get_ushort_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.w d0,16(a5)				*	move.w %d0,16(%a5)	| tmp1041, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.delay_time
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:424:           graphic_ctrl_ext->transparent_index = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,18(a5)				*	move.b %d0,18(%a5)	| tmp1042, MEM <struct GIF_IMAGE_FRAME[1024]> [(struct GIF_GRAPHIC_CONTROL_EXTENSION *)&g_image_frames][image_frame_index_217].graphic_ctrl_ext.transparent_index
...
_?L113:							*.L113:
							*| main.c:562:       fclose(fp);
	move.l d3,-(sp)					*	move.l %d3,-(%sp)	| fp,
	jbsr _fclose					*	jsr fclose		|
							*| main.c:568:       free__(file_buffer_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea _free__,a3					*	lea free__,%a3	|, tmp941
	jbsr (a3)					*	jsr (%a3)		| tmp941
							*| main.c:574:         free__(decode_buffer_ptr, g_high_memory_mode);
	addq.l #4,sp					*	addq.l #4,%sp	|,
	move.l _g_high_memory_mode,(sp)			*	move.l g_high_memory_mode,(%sp)	| g_high_memory_mode,
	move.l -134(a6),-(sp)				*	move.l -134(%fp),-(%sp)	| %sfp,
	jbsr (a3)					*	jsr (%a3)		| tmp941
...
_?L103:							*.L103:
							*| main.c:579:     for (int i = 0; i < image_frame_index; i++) {
	cmp.l -130(a6),d5				*	cmp.l -130(%fp),%d5	| %sfp, image_frame_index
	jble _?L62					*	jle .L62		|
							*| main.c:580:         if (g_image_frames[i].frame_data_ptr != NULL) {
	move.l (a4),d0					*	move.l (%a4),%d0	| MEM[(unsigned char * *)_595], _342
							*| main.c:580:         if (g_image_frames[i].frame_data_ptr != NULL) {
	jbeq _?L114					*	jeq .L114		|
							*| main.c:581:             free__(g_image_frames[i].frame_data_ptr, g_high_memory_mode);
	move.l _g_high_memory_mode,-(sp)		*	move.l g_high_memory_mode,-(%sp)	| g_high_memory_mode,
	move.l d0,-(sp)					*	move.l %d0,-(%sp)	| _342,
	jbsr (a3)					*	jsr (%a3)		| tmp972
	addq.l #8,sp					*	addq.l #8,%sp	|,
							*| main.c:582:             g_image_frames[i].frame_data_ptr = NULL;
	clr.l (a4)					*	clr.l (%a4)	| MEM[(unsigned char * *)_595]
_?L114:							*.L114:
							*| main.c:579:     for (int i = 0; i < image_frame_index; i++) {
...
_?L97:							*.L97:
	...
	move.l a3,-(sp)					*	move.l %a3,-(%sp)	| tmp534,
	move.l d4,-(sp)					*	move.l %d4,-(%sp)	| file_buffer_ptr,
	lea _get_ushort_buffer,a5			*	lea get_ushort_buffer,%a5	|, tmp899
	jbsr (a5)					*	jsr (%a5)		| tmp899
...
_?L99:							*.L99:
	...
	move.w #8703,-98(a6)				*	move.w #8703,-98(%fp)	|, MEM <unsigned short> [(unsigned char *)_410]
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:496:           app_ext.block_size = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-96(a6)				*	move.b %d0,-96(%fp)	| tmp1048, MEM[(struct  *)_410].block_size
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:501:           app_ext.block_size2 = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-84(a6)				*	move.b %d0,-84(%fp)	| _316, MEM[(struct  *)_410].block_size2
	...
	lea (16,sp),sp					*	lea (16,%sp),%sp	|,
							*| main.c:504:           app_ext.terminator = get_uchar_buffer(file_buffer_ptr, &file_buffer_ofs, file_buffer_size, fp);
	move.b d0,-78(a6)				*	move.b %d0,-78(%fp)	| _318, MEM[(struct  *)_410].terminator
...
_?L105:							*.L105:
	...
	lea _VDISPST,a3					*	lea VDISPST,%a3	|, tmp947
	jbsr (a3)					*	jsr (%a3)		| tmp947
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
							*| main.c:540:       if (VDISPST((unsigned char*)output_image_vdisp, 0, 2) == 0) {
	tst.l d0					*	tst.l %d0	| tmp1051
	jbne _?L106					*	jne .L106		|
							*| main.c:542: 	      while (INPOUT(0xFF) == 0) {
	lea _INPOUT,a4					*	lea INPOUT,%a4	|, tmp948
_?L107:							*.L107:
	pea 255.w					*	pea 255.w		|
	jbsr (a4)					*	jsr (%a4)		| tmp948
	addq.l #4,sp					*	addq.l #4,%sp	|,
							*| main.c:542: 	      while (INPOUT(0xFF) == 0) {
	tst.l d0					*	tst.l %d0	| tmp1052
	jbeq _?L107					*	jeq .L107		|
_?L108:							*.L108:
							*| main.c:546:         WAIT_VBLANK;
	move.b 15237121,d0				*	move.b 15237121,%d0	| MEM[(volatile unsigned char *)15237121B], _331
	btst #4,d0					*	btst #4,%d0	|, _331
	jbne _?L108					*	jne .L108		|
_?L109:							*.L109:
							*| main.c:547:         WAIT_VSYNC;
	move.b 15237121,d0				*	move.b 15237121,%d0	| MEM[(volatile unsigned char *)15237121B], _333
	btst #4,d0					*	btst #4,%d0	|, _333
	jbeq _?L109					*	jeq .L109		|
							*| main.c:549:         VDISPST(0, 0, 0);
	clr.l -(sp)					*	clr.l -(%sp)	|
	clr.l -(sp)					*	clr.l -(%sp)	|
	clr.l -(sp)					*	clr.l -(%sp)	|
	jbsr (a3)					*	jsr (%a3)		| tmp947
	lea (12,sp),sp					*	lea (12,%sp),%sp	|,
_?L111:							*.L111:
							*| main.c:555:       WAIT_VSYNC;
	move.b 15237121,d0				*	move.b 15237121,%d0	| MEM[(volatile unsigned char *)15237121B], _335
	btst #4,d0					*	btst #4,%d0	|, _335
	jbeq _?L111					*	jeq .L111		|
_?L112:							*.L112:
							*| main.c:556:       WAIT_VBLANK;
	move.b 15237121,d0				*	move.b 15237121,%d0	| MEM[(volatile unsigned char *)15237121B], _337
	btst #4,d0					*	btst #4,%d0	|, _337
	jbne _?L112					*	jne .L112		|
	jbra _?L113					*	jra .L113		|
_?L106:							*.L106:
							*| main.c:552:         printf("error: cannot use vsync interrupt.\n");
	pea _?LC49					*	pea .LC49		|
	jbsr _puts					*	jsr puts		|
	addq.l #4,sp					*	addq.l #4,%sp	|,
